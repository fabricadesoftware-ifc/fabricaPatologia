...
from laudosMedvet.models import (CidadeModel, RuaModel, BairroModel, EspecieModel, RacaModel,
...
def lista_laudos(request):
    ...
    if por_tipo_laudo:
        filtro.add(Q(id_requisicao__tipo_de_laudo__tipo_laudo__contains=por_tipo_laudo), Q.AND)
    ...
    # if por_bairro:
    #     filtro.add(Q(id_requisicao__cod_animail__rua__id_bairro__nome_bairro__contains=por_bairro), Q.AND)

    # if por_rua:
    #     filtro.add(Q(id_requisicao__cod_animail__rua__nome_rua__icontains=por_rua), Q.AND)
    ...
    estados_ocorrencia = LaudoModel.objects.filter(filtro).distinct(
        'id_requisicao__cod_animail__id_estado').count()
    estados_nomes = LaudoModel.objects.filter(filtro).distinct(
        'id_requisicao__cod_animail__id_estado')
    cidades_ocorrencia = LaudoModel.objects.filter(filtro).distinct(
        'id_requisicao__cod_animail__cidade').count()
    cidades_nome = LaudoModel.objects.filter(filtro).distinct(
        'id_requisicao__cod_animail__cidade__nome_cidade')
    bairros_ocorrencia = LaudoModel.objects.filter(filtro).distinct(
        'id_requisicao__cod_animail__bairro').count()
    ruas_ocorrencia = LaudoModel.objects.filter(filtro).distinct(
        'id_requisicao__cod_animail__rua').count()
    # por_cidades_ocorrencia = LaudoModel.objects.filter(filtro).get(
    #     'id_requisicao__cod_animail__cidade__nome_cidade').count()
    ...
    for caso in estados_nomes:
        estados_lista.append(caso.id_requisicao.cod_animail.id_estado.nome_estado)

    cidades = []
    for caso in cidades_nome:
        cidades.append(caso.id_requisicao.cod_animail.cidade.nome_cidade)
